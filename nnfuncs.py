...
def get_preds_labels(model, optimizer, dataset, device=torch.device('cuda')):
    '''
    torch model, optimizer, torch dataset, optional device -> tensor, tensor, tensor
    Inputs: torch model, optimizer, train or test data, device if wanted
    Outputs: model output, predictions probabilities for class 1, labels
    Iterates over a dataset and returns model output, prediction probabily for class 1
...
def model_accuracy(y_pred: torch.Tensor, label: torch.Tensor, thresh: float):
    ...
    Input: prediction probabilities for class 1, labels, threshold
    Output: accuracy
...
def plot_roc(fpr, tpr, roc_auc, fpath):
    ...
    Inputs: false pos rate, true pos rate, AUROC, filepath
    Outputs: None
    '''
    plt.figure()
    lw = 2
...
def plot_loss(model_id, train_loss, val_loss):
    ...
    Inputs: List of train loss vals, list of validation loss vals
    Output: none
    '''
...
def plot_test_precision_recall(fpath, label, y_pred):
    '''
    Input: filepath, labels, prediciton probabilities
    Output: None
